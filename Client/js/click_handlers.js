(function(){
    ...
    function main(){
        ...
        $(document).on('click', '#btn-search', function(){
            ...
            var url = server + '/spot/' + title;
            $.mobile.loading('show');
            $.getJSON(url).done(renderSpotInfo)
                          .fail(onError);
        });
    ...
    function renderSpotInfo(json) {
        $('#title-search').removeClass('error');
        var template = $('#search-result-template').html();
        var spotSourceContent = $.mustache(template, json);
        var resultDiv = $('#search-result');
        resultDiv.html(spotSourceContent);
    ...
    function onError(jqXHR, status, error){
        ...
        if (jqXHR.status === 404){
            message = "Couldn't find title in database. Please check if it is correct.";
            $('#title-search').addClass('error');
        } else {
    ...
    function postFindingAndGotoMap(){
        ...
        $.mobile.loading('show');
        var tmp = server + '/spot_info';
        console.log('server:', tmp);
        console.log('inputObject:', inputObject);
        var jqxhr = $.post(server + '/spot_info', inputObject);
        jqxhr.done(function() {
            $.mobile.loading('hide');
    ...
    function validateInput(input){
        if (input.title || input.photo){
            $('#title-post').removeClass('error');
            $('#photo-file').removeClass('error');
            return true;
        }
        else{
            showError($('#page-post'), 'Please give either a title or a photo.');
            $('#title-post').addClass('error');
